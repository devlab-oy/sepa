module Sepa
  class SoapBuilder
    ...
    def initialize(params)
      ...
      @type = find_correct_request_type(@bank, @command)
      @ar = ApplicationRequest.new(params).get_as_base64

      @template_path = File.expand_path('../xml_templates/soap/', __FILE__)

      #@request = Sepa::RequestBuilder.new(params) # To be done if deemed good
    end

    def to_xml
      # Returns a complete SOAP message in xml format
      build_soap(@params).to_xml
    end

     def to_xml_unencrypted
       build_danske_create_certificate_request_without_encryption(@params).to_xml
     end
    ...
    private
      ...
      # def check_does_soap_need_to_be_signed()
      # end

      # def check_is_command_supported_by_bank()
      # end
